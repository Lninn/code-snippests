(define (my-eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((and? exp) (eval-and (cdr exp) env))   ;;; and case
        ((or? exp) (eval-or (cdr exp) env))     ;;; or case
        ((application? exp)
...
(define (driver-loop)
	...
	(let ((input (read)))
		(let ((output (my-eval input global)))
...
(define (eval-and exps env)
  (let ((t (my-eval (first-exp exps) env)))
    (if t
        (if (last-exp? exps)
            t
...
case 2
(var (map p items) (if (null? items) '() (cons (p (car items)) (map p (cdr items)))))
(map (lambda (x) x) (list 1 2 3 4))
